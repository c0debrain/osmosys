...
_Ember frontend for [OsmoSys](http://hospitalrun.io/): free software for developing world hospitals_

[![Build Status](https://travis-ci.org/osmosys/hospitalrun-frontend.svg?branch=master)](https://travis-ci.org/osmosys/hospitalrun-frontend) [![CouchDB](https://img.shields.io/badge/couchdb-1.x-green.svg)](http://couchdb.apache.org/)
...
- [Accessing OsmoSys with Docker Toolbox](#accessing-hospitalrun-with-docker-toolbox)
- [Accessing OsmoSys with Docker or Docker-compose](#accessing-hospitalrun-with-docker-or-docker-compose)
...
Contributions are welcome via pull requests and issues.  Please see our [contributing guide](https://github.com/osmosys/hospitalrun-frontend/blob/master/.github/CONTRIBUTING.md) for more details, including a link to join our project Slack.
...
5. Clone this repo with `git clone https://github.com/osmosys/hospitalrun-frontend`, go to the cloned folder and run 
...
6. Install and configure [CouchDB](http://couchdb.apache.org/):
    ...
    4. Create admin user:
        1. If you are running CouchDB 1.x:
            1. If you have just installed CouchDB and have no admin user, please run `./script/initcouch.sh` in the folder you cloned the OsmoSys repo.  A user `hradmin` will be created with password `test`.
            2. If you already have a CouchDB admin user, please run `./script/initcouch.sh USER PASS` in the folder you cloned the OsmoSys repo.  `USER` and `PASS` are the CouchDB admin user credentials.
        2. If you are running CouchDB 2.x (experimental):
            1. OsmoSys currently does not fully support CouchDB 2.x, but you are welcome to try using it.  Most functionality should work but currently creating and/or editing users does not work in CouchDB 2.x.  See https://github.com/osmosys/hospitalrun-frontend/issues/953 for more details.
            2. If you have just installed CouchDB and have no admin user, please run `./script/initcouch2.sh` in the folder you cloned the OsmoSys repo.  A user `hradmin` will be created with password `test`.
            3. If you already have a CouchDB admin user, please run `./script/initcouch2.sh USER PASS` in the folder you cloned the OsmoSys repo.  `USER` and `PASS` are the CouchDB admin user credentials.
7. Copy the `server/config-example.js` to `server/config.js` in the folder you cloned the OsmoSys repo.  If you already had a CouchDB admin user that you passed into the couch script (`./script/initcouch.sh USER PASS`), then you will need to modify the `couchAdminUser` and `couchAdminPassword` values in `server/config.js` to reflect those credentials. (*Note: If on Mac, you need to make sure CouchDB can be run. See [How to open an app from a unidentified developer and exempt it from Gatekeeper](https://support.apple.com/en-us/HT202491).*)
...
To run OsmoSys with [Docker](https://www.docker.com/) please do the following:
- Go to [https://docs.docker.com/engine/installation](https://docs.docker.com/engine/installation) to download and install Docker.
- Clone the repository with the command `git clone https://github.com/osmosys/hospitalrun-frontend.git`.
- Change to the hospitalrun-frontend directory `cd hospitalrun-frontend`.
- Build the OsmoSys image with `docker build -t hospitalrun-frontend .`.
- Execute `docker run -it --name couchdb -d couchdb:1.7.1` to create the couchdb container.
- Execute `docker run -it --name hospitalrun-frontend -p 4200:4200 --link couchdb:couchdb -d hospitalrun-frontend` to create the OsmoSys container.

### Running with Docker Compose
To run OsmoSys with Docker-compose please do the following:
...
### Accessing OsmoSys with Docker Toolbox
...
### Accessing OsmoSys with Docker or Docker-compose
...
Again, contributions are welcome via pull requests and issues.  Please see our [contributing guide](https://github.com/osmosys/hospitalrun-frontend/blob/master/.github/CONTRIBUTING.md) for more details.

**Seriously, please read the [Contribution Guide](https://github.com/osmosys/hospitalrun-frontend/blob/master/.github/CONTRIBUTING.md).**
...
[Documented in the project wiki](https://github.com/osmosys/hospitalrun-frontend/wiki/Optional:-Cloud9-Development-Environment)

## Frequently Asked Questions

**Q: What is the difference between hospitalrun-frontend and hospitalrun-server?**

Frontend is the primary repository and is used for development purposes. Server is the node backend, if you are looking to run OsmoSys in a clinical setting, you should use server.
